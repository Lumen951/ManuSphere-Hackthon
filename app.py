...
def load_config():
    try:
        ...
        # 如果配置文件不存在或缺少必要字段，使用默认配置
        print("使用默认配置: host=127.0.0.1, port=8000")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        # 如果没有命令行参数，启动 Web 服务器
        print(f"启动 Web 服务器: http://{config['host']}:{config['port']}")
